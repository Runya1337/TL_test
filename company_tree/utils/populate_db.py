...
def create_departments_recursive(parent, level, max_level, departments_per_level):
    ...
    for _ in range(departments_per_level.get(level, 1)):
        dept = Department.objects.create(name=fake.company(), parent=parent)
        departments.append(dept)
        logging.debug(f"Создан отдел '{dept.name}' на уровне {level}")
        departments.extend(
            create_departments_recursive(
                dept, level + 1, max_level, departments_per_level
            )
        )
    return departments
...
def create_departments():
    departments_per_level = {1: 5, 2: 4, 3: 2, 4: 1, 5: 1}
    max_level = 5
    ...
    all_departments.extend(
        create_departments_recursive(company, 1, max_level, departments_per_level)
    )
...
def create_employees(n, departments):
    ...
    deepest_departments = [
        dept for dept in departments if not dept.get_children().exists()
    ]
